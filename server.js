...
app.get('/orders', async (req, res) => {
  ...
  try {
    const orders = await pool.query(
      `SELECT 
        id, transport_date, cargo_type, cargo_weight,
        length, width, height, from_address, to_address,
        phone, status, review_rating, review_text
       FROM orders 
       WHERE user_id = $1 
...
// Добавить новую заявку на перевозку
app.post('/orders', async (req, res) => {
  ...
  try {
    ...
    res.status(201).json({ message: 'Заявка на перевозку успешно создана', orderId: result.rows[0].id });
  } catch (error) {
...
app.put('/admin/orders/:id', async (req, res) => {
  const { id } = req.params;
  const { status } = req.body; // Убрали cancelReason

  // Проверяем допустимые статусы
  const allowedStatuses = ['новая', 'в работе', 'выполнено', 'отменено'];
  ...
  try {
    await pool.query(
      'UPDATE orders SET status = $1 WHERE id = $2', // Убрали cancel_reason
      [status, id]
    );
